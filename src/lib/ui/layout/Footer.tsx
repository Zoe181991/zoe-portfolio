...
export function Footer() {
  ...
  return (
    <div
      id="contact"
      className="mt-8 flex flex-col w-full px-8 md:px-16 py-12 bg-base-1 items-center"
    >
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 w-full max-w-3xl mb-10">
        <Reveal className={`flex flex-col gap-4 ${isHebrew ? "md:items-end md:text-right" : "md:items-start md:text-left"} items-center text-center`}>
        ...
        <Reveal delay={120} className={`flex flex-col gap-4 ${isHebrew ? "md:items-end md:text-right" : "md:items-start md:text-left"} items-center text-center`}>
      ...
      <div className="text-xs text-base-4 text-opacity-60">{copyright}</div>
    </div>
  );
}
