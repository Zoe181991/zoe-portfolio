                    ...
                    return (
                        ...
                        <div key={index} id={`slide${index}`} className="carousel-item relative w-full">
                            ...
                            <div className="absolute flex justify-between transform -translate-y-1/2 left-5 right-5 top-1/2">
                                <a onClick={()=>handlePrevious(index)} className="btn btn-circle opacity-60">❮</a>
                                <a onClick={()=>handleNext(index)} className="btn btn-circle opacity-60">❯</a>
                            </div>
                                        ...
                                        <div key={indexModal} className={(indexModal===activeIndexModal)? " w-full opacity-100 duration-700 ease-in-out " : "hidden"}>
                                            ...
                                            <Image  src={image.src} alt={image.text} layout="responsive" width={680} height={480} />
                                                <div className="absolute flex opacity-60 justify-between transform -translate-y-1/2 left-5 right-5 top-1/2">
                                                    <a onClick={handlePreviousModal}  className="btn btn-circle">❮</a>
                                                    <a onClick={handleNextModal} className="btn btn-circle">❯</a>
                                                </div>
                                            </div>
                                        </div>
